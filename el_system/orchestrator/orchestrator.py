...
class Orchestrator:
    ...
    def run_concurrent_job(self, file_path, job_name):
        ...
        try:
            ...
            log.error(
                f"Job Execution: {job_name} | ID: {job_run_id} | Preparations Failed"
            )
        ...
        try:
            ...
            log.info(
                f"Job Execution: {metadata.job_name} | ID: {job_run_id} | System: {metadata.system} | Job Type: {metadata.job_type} | Sub JobType: {metadata.sub_jobtype} | Status: RUNNING..."
            )
            ...
            log.error(
                f"Job Execution: {metadata.job_name} | ID: {job_run_id} | System: {metadata.system} | Job Type: {metadata.job_type} | Sub JobType: {metadata.sub_jobtype}"
            )
        ...
        with tpe(max_workers=5) as executor:
            for job in job_catalog_loader.jobs:
                ...
                future = executor.submit(
                    orchestrator.run_concurrent_job, file_path, job_name
                )
